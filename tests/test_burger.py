...
def test_move_ingredient(prepared_burger):
...
def test_get_price(prepared_burger):
...
def test_get_receipt(prepared_burger):
...
def test_add_multiple_ingredients(empty_burger, ingredient_type):
...
def test_get_price_with_mocked_bun_and_ingredients(empty_burger):
...
def test_get_receipt_with_mocked_methods(prepared_burger):
    with patch.object(Bun, 'get_name', return_value="Моковая булочка") as mock_bun_name:
        with patch.object(Ingredient, 'get_name', side_effect=["Моковый соус", "Моковая начинка"]) as mock_ingredient_name:
